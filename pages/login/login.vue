<template>
	<view class="flex flex-direction pageview bg-white">
		<cu-custom :isHome="true"><view slot="content">登录</view></cu-custom>
		<scroll-view scroll-x class="bg-white nav text-center margin-top">
			<view class="cu-item" :class="index == TabCur ? 'text-orange cur' : ''" v-for="(item, index) in 2" :key="index" @tap="tabSelect" :data-id="index">
				{{ index == 0 ? '密码登录' : '快捷登录' }}
			</view>
		</scroll-view>
		<view class="flex-sub">
			<form class="margin-top">
				<view class="cu-form-group">
					...
					<input placeholder="输入注册时用的手机号码" name="input" @blur="changeMobile" v-model.lazy="registeMobile"/>
					<image v-if="isCheckmobile" src="#" mode="widthFix" class="loading text-orange"></image>
				</view>
				<view class="cu-form-group" v-if="TabCur == 1">
					<view class="title">验证码</view>
					<input placeholder="输入验证码" name="input" v-model.lazy="verifyNum" />
				...
				<view class="cu-form-group solid-bottom" v-if="TabCur == 0">
					...
					<input placeholder="5~20位英文、数字或符号，区分大小写" name="input" type="password" v-model="password" />
				</view>
			</form>
			...
			<view class="margin-lg flex justify-between">
				<navigator hover-class="none" class="text-orange" url="/pages/reg/reg">
					<text class="text-grey">还没有注册账号？</text>
					<text>立即注册</text>
		...
		<view class="cu-modal" :class="showModal ? 'show' : ''">
			<view class="cu-dialog">
				...
				<view class="cu-bar bg-white"><view class="action margin-0 flex-sub  solid-left" @tap="hideModal">扫描二维码登录</view></view>
			</view>
		</view>
		<!-- <view class="otherway">
...
<script>
import { regPhone, regLoginPwd } from '@/utils/index';
...
export default {
	...
	onLoad() {
		try {
			this.registeMobile = uni.getStorageSync('registeMobile');
			...
			if (this.isAble) {
				...
				this.$postajax(
					...
					if (da.code == 10000) {
						...
						uni.setStorageSync('registeMobile', this.registeMobile);
						if (param.password) {
							uni.setStorageSync('password', param.password);
						}
