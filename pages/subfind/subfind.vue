<template>
	<view class="pageview flex flex-direction">
		<topsearchbar :isBack="true" bgColor="bgColor" @backPage="BackPage">
			...
			<view slot="content"><searchbar @search="search"></searchbar></view>
		</topsearchbar>
		<scroll-view scroll-x v-if="menulist.length > 0" class="bread nav" scroll-with-animation :scroll-into-view="intonav">
			<div ref="bread">
		...
		<view class="bg-gradual-orange currentNode">
			<view class="name">{{ currentNode.name }}</view>
			<view class="margin-top-sm" v-if="currentNode.description">{{ currentNode.description }}</view>
			<view @tap="collection" class="follow flex align-center justify-center">
		...
		findPageMaterials() {
			...
			this.$postajax(this.$api.findPageMaterials, param).then(da => {
				if (da.code == 10000) {
					...
					if ($me.mescroll.num == 1) $me.materialslist = []; //如果是第一页需手动制空列表
...
.currentNode {
	...
	box-shadow: 0 10upx 6upx rgba($color: #000000, $alpha: .1);
	position: relative;
	z-index: 99;
	text-align: center;
	font-size: 20upx;
